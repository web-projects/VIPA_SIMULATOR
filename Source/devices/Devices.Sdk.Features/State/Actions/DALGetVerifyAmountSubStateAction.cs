...
{
    internal class DALGetVerifyAmountSubStateAction : DALBaseSubStateAction
    {
        public override DALSubWorkflowState WorkflowStateType => GetVerifyAmount;

        public DALGetVerifyAmountSubStateAction(IDALSubStateController _) : base(_) { }
            ...
            else
            {
                ...
                if (targetDevice is ICardDevice cardDevice)
                {
                    ...
                    var timeoutPolicy = await cancellationBroker.ExecuteWithTimeoutAsync<LinkRequest>(
                        _ => cardDevice.GetVerifyAmount(linkRequest, _),
                    ...
                    {
                        _ = Controller.LoggingClient.LogErrorAsync($"Unable to obtain verify amount choice from device - '{Controller.DeviceEvent}'.", StatusType.DALTimeOuts);
