...
{
    internal class DALGetPinSubStateAction : DALBaseSubStateAction
    {
        public override DALSubWorkflowState WorkflowStateType => GetPin;

        public DALGetPinSubStateAction(IDALSubStateController _) : base(_) { }
        ...
        public override async Task DoWork()
        {
            ...
            {
                _ = Controller.LoggingClient.LogErrorAsync("Unable to find a state object while attempting to get PIN from device.");
                _ = Error(this);
            }
            else
            {
                ...
                if (targetDevice is ICardDevice cardDevice)
                {
                    ...
                    var timeoutPolicy = await cancellationBroker.ExecuteWithTimeoutAsync<LinkRequest>(
                        _ => cardDevice.GetPin(linkRequest, _),
                    ...
                    {
                        _ = Controller.LoggingClient.LogErrorAsync($"Unable to obtain PIN from device - '{Controller.DeviceEvent}'.", StatusType.DALTimeOuts);
